...
constexpr unsigned maximum_json_depth = 64;
constexpr int maximum_download_hops = 6;
constexpr int network_timeout_ms = 3000;
constexpr unsigned maximum_archive_entries = 4096;
...
void check_stop(std::stop_token stop, clock_type::time_point deadline)
{
	...
	if (clock_type::now() > deadline)
		throw std::runtime_error("Update operation timed out.");
}
...
struct cancellable_request
{
	...
	void close()
	{
		if (const auto handle = value.exchange(nullptr))
...
std::vector<std::byte> download(
	std::wstring_view input, std::size_t limit, std::stop_token stop, const progress_callback& progress)
{
	...
	if (!WinHttpSetTimeouts(
			session.value, network_timeout_ms, network_timeout_ms, network_timeout_ms, network_timeout_ms))
		throw std::runtime_error(win_error("Cannot configure HTTPS timeouts"));
	std::wstring url(input);
	for (int hop = 0; hop != maximum_download_hops; ++hop)
	{
		...
		if (!connection.value)
			throw std::runtime_error(win_error("Cannot connect to GitHub"));
		cancellable_request request{WinHttpOpenRequest(connection.value, L"GET", path.c_str(), nullptr,
			WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, WINHTTP_FLAG_SECURE)};
		if (!request.value)
			throw std::runtime_error(win_error("Cannot create update request"));
		...
		{
			DWORD bytes = 0;
			WinHttpQueryHeaders(request.value, WINHTTP_QUERY_LOCATION, WINHTTP_HEADER_NAME_BY_INDEX, nullptr, &bytes,
				WINHTTP_NO_HEADER_INDEX);
			if (!bytes || bytes > 32768)
				throw std::runtime_error("Invalid update redirect.");
		...
		if (declared > limit)
			throw std::runtime_error("Update download exceeds its size limit.");
		std::vector<std::byte> result;
		if (declared)
			result.reserve(declared);
		...
		for (;;)
		{
			...
			if (!received)
				break;
			if (received > limit - result.size())
				throw std::runtime_error("Update download exceeds its size limit.");
			result.insert(result.end(), buffer.begin(), buffer.begin() + received);
			if (progress)
				progress(declared ? static_cast<float>(result.size()) / declared : 0.0f);
		}
		check_stop(stop, deadline);
		if (declared && declared != result.size())
...
{
	if (directory.empty() || !ordinary_directory(directory))
		return;
	...
	{
		if (ordinary_file(directory / L"owner.txt"))
			DeleteFileW((directory / L"owner.txt").c_str());
		if (ordinary_file(directory / L"installed.txt"))
			DeleteFileW((directory / L"installed.txt").c_str());
		RemoveDirectoryW(directory.c_str());
	}
}
void cleanup_previous_stages(const std::filesystem::path& executable)
{
	if (!ordinary_file(executable))
		return;
	...
	for (const auto& entry : std::filesystem::directory_iterator(executable.parent_path(), ec))
	{
		...
		if (!name.starts_with(L".safc-update-") || !ordinary_directory(entry.path()))
			continue;
		const auto marker = entry.path() / L"owner.txt";
		if (!ordinary_file(marker) || std::filesystem::file_size(marker, ec) > 65536 || ec)
			continue;
		const auto installed_marker = entry.path() / L"installed.txt";
		if (!ordinary_file(installed_marker) || std::filesystem::file_size(installed_marker, ec) > 32 || ec)
			continue;
		...
		if (!installed_version || *current < *installed_version)
			continue;
		...
		if (owner == expected && ordinary_file(entry.path() / L"previous.exe"))
			cleanup_stage(entry.path());
	}
}
std::filesystem::path make_stage(const std::filesystem::path& executable)
{
	for (unsigned attempt = 0; attempt != 32; ++attempt)
	{
		...
		if (CreateDirectoryW(path.c_str(), nullptr))
		{
			...
			{
				cleanup_stage(path);
				throw std::runtime_error("Cannot record update staging ownership.");
			}
			return path;
		}
...
{
	const auto result = download(release_endpoint, maximum_metadata_bytes, stop, {});
...
std::optional<version> parse_version(std::string_view text)
{
	...
	for (std::size_t part = 0; part != value.size(); ++part)
	{
		...
		if (digits.empty() || digits.size() > 5 || (digits.size() > 1 && digits.front() == '0'))
			return {};
		unsigned number = 0;
...
{
	return "v" + std::to_string(value[0]) + "." + std::to_string(value[1]) + "." + std::to_string(value[2]) + "." +
		std::to_string(value[3]);
}
std::optional<release> select_release(std::string_view json, std::uint16_t machine, std::string& error)
{
	error.clear();
	auto fail = [&](const char* message) -> std::optional<release>
	{
		error = message;
		return {};
	};
	if (json.empty() || json.size() > maximum_metadata_bytes || json.find('\0') != json.npos)
	...
	for (char character : json)
	{
		...
		else if (character == '[' || character == '{')
		{
			if (++depth > maximum_json_depth)
				return fail("Release metadata is too deeply nested.");
		}
		else if (character == ']' || character == '}')
		{
			if (!depth)
	...
	if (!value || !value->IsObject())
		return fail("GitHub returned invalid release metadata.");
	for (const auto* flag : {L"draft", L"prerelease"})
	{
		const auto* child = value->Child(flag);
		if (!child || !child->IsBool() || child->AsBool())
			return fail("Only published stable releases can be installed.");
	}
	...
	if (!published || !published->IsString() || published->AsString().empty() || !tag || !tag->IsString() || !assets ||
		!assets->IsArray())
		return fail("Release metadata is missing required fields.");
	std::string tag_text;
	tag_text.reserve(tag->AsString().size());
	for (const wchar_t character : tag->AsString())
	{
		if (character > 127)
			return fail("Release tag must contain only ASCII version characters.");
		tag_text.push_back(static_cast<char>(character));
	}
	const auto number = parse_version(tag_text);
	if (!number)
		return fail("Release tag must be vN.N.N.N.");
	const wchar_t* wanted = machine == machine_x64 ? L"SAFC64.7z" : machine == machine_x86 ? L"SAFC32.7z" : nullptr;
	if (!wanted)
		return fail("Unsupported update architecture.");
	...
	for (auto* asset : assets->AsArray())
	{
		...
		if (!name || !name->IsString() || name->AsString() != wanted)
			continue;
		if (selected)
			return fail("Release contains duplicate architecture assets.");
		...
		if (!url || !url->IsString() || url->AsString() != expected || !size || !size->IsNumber() ||
			...
			std::floor(size->AsNumber()) != size->AsNumber() || !state || !state->IsString() ||
			state->AsString() != L"uploaded")
	...
	if (!selected)
		return fail("The release has no update archive for this architecture.");
	return selected;
}
std::optional<version> executable_version(const std::filesystem::path& executable)
{
	...
	if (!GetFileVersionInfoW(executable.c_str(), 0, size, bytes.data()))
		return {};
	VS_FIXEDFILEINFO* info = nullptr;
	UINT length = 0;
	if (!VerQueryValueW(bytes.data(), L"\\", reinterpret_cast<void**>(&info), &length) || length < sizeof(*info) ||
		info->dwSignature != 0xfeef04bd)
		return {};
	return version{HIWORD(info->dwProductVersionMS), LOWORD(info->dwProductVersionMS), HIWORD(info->dwProductVersionLS),
		LOWORD(info->dwProductVersionLS)};
}
bool verify_executable(
	const std::filesystem::path& executable, const version& expected, std::uint16_t machine, std::string& error)
{
	error.clear();
	if (!ordinary_file(executable))
	{
		error = "Staged update is not an ordinary executable file.";
		return false;
	}
	...
	{
		error = "Staged update has an invalid DOS executable header.";
		return false;
	}
	...
	{
		error = "Staged update has the wrong executable architecture.";
		return false;
	}
	const auto actual = executable_version(executable);
	if (!actual || *actual != expected)
	{
		error = "Staged executable ProductVersion does not match its release tag.";
		return false;
	}
	return true;
}
bool extract_executable(
	std::span<const std::byte> bytes, const std::filesystem::path& output, std::stop_token stop, std::string& error)
{
	error.clear();
	bool created = false;
	try
	{
		...
		check_stop(stop, deadline);
		if (bytes.empty() || bytes.size() > max_archive_bytes)
			throw std::runtime_error("Invalid update archive size.");
		std::unique_ptr<archive, decltype(&archive_read_free)> reader(archive_read_new(), archive_read_free);
		if (!reader)
			throw std::runtime_error("Cannot create update archive reader.");
		...
		for (;;)
		{
			...
			if (result == ARCHIVE_EOF)
				break;
			if (result != ARCHIVE_OK || ++entries > maximum_archive_entries)
				throw std::runtime_error("Invalid update archive headers.");
			...
			for (std::size_t offset = 0; offset < path.size();)
			{
				...
				if (part == ".." || part == "." || part.empty())
					throw std::runtime_error("Update archive contains a traversal path.");
				if (end == path.npos)
					break;
				offset = end + 1;
			}
			const auto kind = archive_entry_filetype(entry);
			if (kind != AE_IFREG && kind != AE_IFDIR)
				throw std::runtime_error("Update archive contains a special file.");
			...
			if (lower != "safc.exe")
			{
				if (lower.ends_with("/safc.exe"))
					throw std::runtime_error("Update executable must be at the archive root.");
				if (archive_read_data_skip(reader.get()) != ARCHIVE_OK)
					throw std::runtime_error("Cannot skip update archive member.");
				continue;
			}
			...
			found = true;
			file_handle file{
				CreateFileW(output.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, nullptr)};
			if (file.value == INVALID_HANDLE_VALUE)
				throw std::runtime_error(win_error("Cannot stage update executable"));
			created = true;
			std::array<std::byte, 64 * 1024> buffer;
			std::size_t total = 0;
			for (;;)
			{
				...
				if (count < 0)
					throw std::runtime_error("Update executable decompression failed.");
				if (!count)
					break;
				...
				DWORD written = 0;
				if (!WriteFile(file.value, buffer.data(), static_cast<DWORD>(count), &written, nullptr) ||
					written != count)
					throw std::runtime_error(win_error("Cannot write update executable"));
				total += count;
			}
			if (total != archive_entry_size(entry))
				throw std::runtime_error("Update executable size is inconsistent.");
			if (!FlushFileBuffers(file.value))
				throw std::runtime_error(win_error("Cannot flush update executable"));
		}
		if (!found)
			throw std::runtime_error("Update archive has no root SAFC.exe.");
		check_stop(stop, deadline);
		return true;
	}
...
{
	error.clear();
	if (!target.is_absolute() || !staged.is_absolute() || !backup.is_absolute() || !ordinary_file(target) ||
		!ordinary_file(staged) || std::filesystem::exists(backup) || !ordinary_directory(staged.parent_path()) ||
		!ordinary_directory(backup.parent_path()) || target == staged || target == backup)
	{
		error = "Update installation paths are invalid or already occupied.";
		return false;
	}
	if (ReplaceFileW(target.c_str(), staged.c_str(), backup.c_str(), REPLACEFILE_IGNORE_MERGE_ERRORS, nullptr, nullptr))
		return true;
	...
	{
		error = win_error("Cannot move the previous executable aside");
		return false;
	}
	if (MoveFileExW(staged.c_str(), target.c_str(), MOVEFILE_WRITE_THROUGH))
		return true;
	const auto install_error = GetLastError();
	if (!MoveFileExW(backup.c_str(), target.c_str(), MOVEFILE_WRITE_THROUGH))
	{
		error = win_error(
			"Update replacement and rollback failed; the previous executable remains in the update backup folder",
			install_error);
		return false;
	}
...
struct update_service::impl
{
	...
	explicit impl(source source)
		: executable(source.executable.empty() ? self_path() : std::filesystem::absolute(source.executable)),
		  ...
		  install(source.install ? std::move(source.install) : replace_executable)
	{
		...
		status.message = current ? "Updates are checked against published stable GitHub releases."
								 : "The executable has no valid ProductVersion resource.";
	}
	void run(std::stop_token stop)
	{
		...
		{
			if (!current)
				throw std::runtime_error("Cannot check updates without the executable ProductVersion resource.");
			const auto metadata = fetch(release_endpoint, maximum_metadata_bytes, stop, {});
			if (stop.stop_requested())
				throw std::runtime_error("Update cancelled.");
			std::string error;
			const auto selected =
				select_release(std::string_view(reinterpret_cast<const char*>(metadata.data()), metadata.size()),
					native_machine, error);
			if (!selected)
			...
			const auto archive =
				fetch(selected->url, std::min(selected->size, max_archive_bytes), stop, [&](float progress)
			{
				std::lock_guard lock(mutex);
			...
			if (stop.stop_requested())
				throw std::runtime_error("Update cancelled.");
			if (archive.size() != selected->size)
				throw std::runtime_error("Update archive size does not match the release metadata.");
...
{
	shutdown();
	if (!p_->preserve_backup)
		cleanup_stage(p_->stage);
}
update_snapshot update_service::snapshot() const
{
...
void update_service::check()
{
	{
		std::lock_guard lock(p_->mutex);
		if (p_->status.phase == update_phase::checking || p_->status.phase == update_phase::downloading ||
			p_->status.phase == update_phase::ready)
			return;
	}
	...
	try
	{
		cleanup_previous_stages(p_->executable);
	}
	catch (...)
	{
	}
...
bool update_service::install_pending(std::string& error)
{
	...
	if (!p_->pending)
		return false;
	if (!verify_executable(p_->stage / L"SAFC.exe", *p_->pending, native_machine, error))
		return false;
...
std::filesystem::path update_service::executable_path() const
{
