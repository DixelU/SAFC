...
std::wstring decode_utf8(std::string bytes)
{
	...
	const auto length =
		MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, bytes.data(), static_cast<int>(bytes.size()), nullptr, 0);
	if (!length)
		throw std::runtime_error("The JSON config must contain valid UTF-8 text.");
	std::wstring wide(length, L'\0');
	MultiByteToWideChar(
		CP_UTF8, MB_ERR_INVALID_CHARS, bytes.data(), static_cast<int>(bytes.size()), wide.data(), length);
	return wide;
}
...
struct numeric_lexemes
{
	...
	void whitespace()
	{
		while (position < source.size() && iswspace(source[position]))
			++position;
	}
	wchar_t peek()
	{
	...
	{
		if (depth > 64)
			throw std::runtime_error("JSON nesting exceeds 64 levels.");
		const auto c = peek();
		if (c == L'{')
		{
			++position;
			...
			for (;;)
			{
				...
				if (!key || !key->IsString() || peek() != L':')
					throw std::runtime_error("Invalid JSON object.");
				++position;
				scan(path + L"/" + escaped_path(key->AsString()), depth + 1);
				if (peek() == L'}')
				{
					++position;
					return;
		...
		while (position < source.size() && !iswspace(source[position]) && source[position] != L',' &&
			source[position] != L']' && source[position] != L'}')
			++position;
		if (start == position)
		...
		if (c == L'-' || (c >= L'0' && c <= L'9'))
		{
			...
			if (const auto exponent = token.find_first_of(L"eE"); exponent != std::wstring::npos)
			{
				...
				if (magnitude < -308 || magnitude > 308)
					invalid(path, "exponent must be within -308..308");
			}
			values[path] = token;
		}
	}
...
struct config_reader
{
		...
		std::int64_t minimum, std::int64_t maximum) const
	{
		...
		if (!value->IsNumber())
			invalid(path, "must be an integer number");
		...
		if (token.find_first_of(L".eE") == std::wstring::npos)
		{
			...
			if (parsed.ec != std::errc{} || parsed.ptr != narrow.data() + narrow.size())
				invalid(path, "integer is outside signed 64-bit range");
		}
		else
		{
	...
	static std::optional<double> number(
		const JSONObject& object, const wchar_t* key, const std::wstring& prefix, double maximum)
	{
		const auto* value = field(object, key);
		if (!value)
			return {};
		if (!value->IsNumber() || !std::isfinite(value->AsNumber()) || value->AsNumber() < 0 ||
			value->AsNumber() > maximum)
			invalid(
				prefix + L"/" + key, "must be a finite nonnegative number no greater than " + std::to_string(maximum));
	...
	static std::optional<bool> boolean(const JSONObject& object, const wchar_t* key, const std::wstring& prefix)
	{
		...
		if (!value)
			return {};
		if (!value->IsBool())
			invalid(prefix + L"/" + key, "must be true or false");
		return value->AsBool();
	}
...
void validate_midi(const std::wstring& filename, const std::wstring& field)
{
	...
	if (!input || header[0] != 'M' || header[1] != 'T' || header[2] != 'h' || header[3] != 'd' || header[4] ||
		header[5] || header[6] || header[7] != 6 || (!header[10] && !header[11]))
...
{
	if (project.loading() || project.merging() || !project.data.files.empty())
		throw std::runtime_error("CLI config requires an empty project.");
	std::ifstream input(std::filesystem::path(path), std::ios::binary);
	if (!input)
		throw std::runtime_error("Cannot open JSON config: " + utf8(path));
	const std::string bytes((std::istreambuf_iterator<char>(input)), {});
	const auto wide = decode_utf8(bytes);
	numeric_lexemes lexemes{wide};
	lexemes.scan();
	if (lexemes.peek())
		throw std::runtime_error("Unexpected data after the JSON config.");
	...
	if (offset_alias && offset_old && *offset_alias != *offset_old)
		throw std::runtime_error("global_offset and global_offset_override conflict.");
	...
	if (!files || !files->IsArray() || files->AsArray().empty())
		throw std::runtime_error("files must be a nonempty array.");

	const std::pair<const wchar_t*, std::uint32_t> flags[] = {{L"ignore_notes", _BoolSettings::ignore_notes},
		{L"ignore_pitches", _BoolSettings::ignore_pitches}, {L"ignore_tempos", _BoolSettings::ignore_tempos},
		{L"ignore_other", _BoolSettings::ignore_all_but_tempos_notes_and_pitch},
	...
	const std::pair<const wchar_t*, bool file_settings::*> switches[] = {
		...
		{L"apply_offset_after", &file_settings::apply_offset_after},
		{L"rsb_compression", &file_settings::rsb_compression},
		{L"ignore_meta_rsb", &file_settings::allow_legacy_rsb_meta_interaction},
		{L"inplace_mergable", &file_settings::inplace_merge_enabled}, {L"allow_sysex", &file_settings::allow_sysex},
		{L"enable_zero_velocity", &file_settings::enable_zero_velocity}};
	...
	for (std::size_t i = 0; i < files->AsArray().size(); ++i)
	{
		...
		if (!filename)
			invalid(prefix + L"/filename", "is required");
		...
		if (length >= 0 && start > INT64_MAX - length)
			invalid(prefix, "selection end exceeds signed 64-bit range");
		for (const auto& [name, flag] : flags)
			if (const auto setting = reader.boolean(object, name, prefix))
				entry.flags.emplace_back(flag, *setting);
		for (const auto& [name, member] : switches)
			if (const auto setting = reader.boolean(object, name, prefix))
				entry.switches.emplace_back(member, *setting);
		validate_midi(entry.filename, prefix + L"/filename");
		entries.push_back(std::move(entry));
	}

	auto output = save_to.value_or(entries.front().filename + L".AfterSAFC.mid");
	if (_wcsicmp(std::filesystem::path(output).extension().c_str(), L".mid") != 0)
		output += L".mid";
	...
	for (const auto& entry : entries)
	{
		...
		if (_wcsicmp(canonical_input.c_str(), canonical_output.c_str()) == 0)
			throw std::runtime_error("The merge output must not overwrite an input MIDI.");
	}
	const auto defaults = load_preferences ? preferences_store{}.load() : application_preferences{};
	project.set_defaults(defaults);
	std::vector<std::wstring> paths;
	for (const auto& entry : entries)
		paths.push_back(entry.filename);
	project.add_files(std::move(paths));
	while (project.loading())
		std::this_thread::sleep_for(std::chrono::milliseconds(2));
	project.poll();
	if (project.data.files.size() != entries.size())
		throw std::runtime_error(
			"An input MIDI could not be loaded; no per-file settings were applied. " + project.message());
	project.data.is_cli_mode = true;
	if (global_ppq)
		project.data.set_global_ppqn(static_cast<std::uint16_t>(*global_ppq), true);
	if (global_tempo)
		project.data.set_global_tempo(static_cast<float>(*global_tempo));
	if (global_offset)
		project.data.set_global_offset(static_cast<std::int32_t>(*global_offset));
	for (std::size_t i = 0; i < entries.size(); ++i)
	{
		...
		if (entry.ppq)
		{
			file.new_ppqn = static_cast<std::uint16_t>(*entry.ppq);
			file.ppqn_manually_set = true;
		}
		if (entry.tempo)
			file.new_tempo = *entry.tempo;
		if (entry.offset)
			file.offset_ticks = *entry.offset;
		if (entry.selection_start)
			file.selection_start = *entry.selection_start;
		if (entry.selection_length)
			file.selection_length = *entry.selection_length;
		for (const auto& [flag, state] : entry.flags)
			file.set_bool_setting(flag, state);
		for (const auto& [member, state] : entry.switches)
			file.*member = state;
	}
...
int run_cli(const std::wstring& argument, bool load_preferences)
{
	...
	{
		std::cout << cli_help() << std::flush;
		return 0;
	}
	if (argument.empty())
	{
		std::cerr << cli_help() << std::flush;
		return 2;
	}
	try
	{
		...
		while (project.merging())
		{
			...
			if (previous_stage != progress.stage)
			{
				...
				std::cout << progress.stage << '\n' << std::flush;
			}
			std::this_thread::sleep_for(std::chrono::milliseconds(40));
		}
		const auto finished = project.progress();
		if (!finished.error.empty())
			throw std::runtime_error(finished.error);
		if (finished.stage != "Merge complete")
			throw std::runtime_error(finished.stage);
		std::cout << "Merge complete in " << finished.seconds << " seconds.\n" << std::flush;
		return 0;
	}
	catch (const std::exception& error)
	{
		std::cerr << "SAFC CLI: " << error.what() << '\n' << std::flush;
		return 1;
	}
}
}
