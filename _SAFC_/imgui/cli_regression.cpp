...
{
	const unsigned char midi[] = {'M', 'T', 'h', 'd', 0, 0, 0, 6, 0, 0, 0, 1, 1, 0xe0, 'M', 'T', 'r', 'k', 0, 0, 0, 13,
...
void reject(const std::filesystem::path& config, const std::string& text, const char* expected)
{
	...
	try
	{
		load_cli_config(config.wstring(), project, false);
	}
...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		write_config(config,
		    "{\"global_ppq_override\":960,\"global_tempo_override\":120,\"global_offset_override\":27," + save +
		        ",\"files\":[{" + filename +
		        ",\"ppq_override\":240,\"tempo_override\":90,\"offset\":9007199254740993," +
		        "\"piano_only\":false,\"remove_empty_tracks\":true,\"allow_sysex\":true},{" + filename + "}]}");
		{
			...
			require(project.data.global_ppqn == 960 && project.data.files[0].new_ppqn == 240 &&
			        project.data.files[1].new_ppqn == 960,
			    "Global/per-file PPQN precedence changed.");
			require(project.data.files[0].new_tempo == 90 && project.data.files[1].new_tempo == 120,
			    "Global/per-file tempo precedence changed.");
			require(
			    project.data.files[0].offset_ticks == 9007199254740993LL && project.data.files[1].offset_ticks == 27,
			    "Tick integer precision or offset precedence was lost.");
			require(!(project.data.files[0].bool_settings & _BoolSettings::all_instruments_to_piano),
			    "remove_empty_tracks incorrectly enabled piano-only.");
			require(project.data.files[0].bool_settings & _BoolSettings::remove_empty_tracks,
			    "remove_empty_tracks flag was not applied.");
			require(project.data.files[0].allow_sysex, "Boolean switch was not applied.");
			require(project.data.save_path == output.wstring() + L".mid",
			    "Missing extension was not appended exactly once.");
		}
		write_config(config,
		    "{\"global_offset\":-3,\"save_to\":" + quote(directory / "already.MID") + ",\"files\":[" + entry + "]}");
		{
			project_session project;
			load_cli_config(config.wstring(), project, false);
			require(project.data.files[0].offset_ticks == -3, "Legacy global_offset alias failed.");
			require(std::filesystem::path(project.data.save_path).filename() == L"already.MID",
			    "Existing uppercase MID extension was duplicated.");
		}
		...
		reject(config,
		    "{\"files\":[{" + filename + ",\"selection_start\":9223372036854775807,\"selection_length\":1}]}",
		    "selection end");
		reject(config,
		    "{\"files\":[{\"filename\":" + quote(directory / "missing.mid") + ",\"ignore_notes\":true}," + entry + "]}",
		    "/files/0/filename");
		...
		write_config(merge_config,
		    "{" + save + ",\"files\":[{" + filename + ",\"piano_only\":false,\"remove_remnants\":true}]}");
		...
		char magic[4]{};
		merged_input.read(magic, 4);
		require(std::string(magic, 4) == "MThd" && std::filesystem::file_size(merged) > 22,
		    "CLI did not write a valid MIDI output.");
		...
		std::cout << "CLI regression passed: Unicode, defaults bypass, all-or-nothing validation, integer precision, "
		             "per-file precedence, aliases, extension, flags, and actual MIDI merge.\n";
		return 0;
	}
	catch (const std::exception& error)
	{
