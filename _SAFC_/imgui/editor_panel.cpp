...
struct editor_panel::impl
{
	...
	void finish_gesture(bool apply)
	{
		...
		if (apply)
		{
			...
			else if (gesture == gesture_kind::move && (delta_tick || delta_key))
	...
	void draw_tool()
	{
		...
		if (begin_folded_window(title, &open))
		{
			...
			else
			{
				...
				if (lane == 4)
					ImGui::TextWrapped("LFO supports velocity, pitch bend, pan and channel volume. Choose one of those lanes first.");
	...
	{
		auto& io = ImGui::GetIO();
		if (!ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) || io.WantTextInput || ImGui::IsAnyItemActive() || busy)
			return;
	...
	void draw_tracks()
	{
		...
		if (named_track != document->get_active_track())
		{
			...
			if (found != document->get_tracks().end())
				std::copy_n(
					found->second.name.data(),
					std::min(found->second.name.size(), track_name.size() - 1),
					track_name.data());
		}
	...
	void draw_canvas(bool interactive)
	{
		...
		while (double(grid) / double(duration) * width < 14.)
			grid *= 2;
		...
		if (gesture == gesture_kind::move || gesture == gesture_kind::resize || gesture == gesture_kind::stretch)
		{
			for (const auto& value : gesture_notes)
			{
				...
				{
					first = static_cast<double>(stretch_begin) + (first - static_cast<double>(stretch_begin)) * stretch_factor;
					last = static_cast<double>(stretch_begin) + (last - static_cast<double>(stretch_begin)) * stretch_factor;
				}
		...
		else if (gesture == gesture_kind::draw)
		{
			note value(
				anchor_tick,
				std::max(anchor_tick + gesture_length, snapped(current_tick) + snap_ticks()),
				std::uint8_t(current_key),
				std::uint8_t(draw_velocity),
				std::uint8_t(channel()),
		...
		if (show_lane)
		{
			...
			if (lane == 0)
			{
				for (const auto& value : document->get_notes_in_range(start, start + duration))
				{
					...
					const float x = 
						x_at(double(value.start_tick)),
							y = lane_y(pending_velocity == velocities.end() ? 
								value.velocity :
			...
			else
			{
				...
				if (lane == 4)
					points = document->get_tempo_points(start, start + duration);
				else
				{
					auto control_points = document->get_channel_control_points(
						document->get_active_track(),
						std::uint8_t(channel()),
						control_lane(),
						start,
				...
				if (!first)
					draw->AddLine(previous, ImVec2(lane_maximum.x, previous.y), IM_COL32(70, 174, 229, 255), 2.f);
		...
		if (interactive && (hovered || active_item || gesture != gesture_kind::none))
		{
				...
				else if (in_roll && ImGui::IsMouseClicked(ImGuiMouseButton_Left))
				{
					...
					else if (found)
					{
						...
						if (!document->is_note_selected(hit.id))
							document->select_note(hit.id, io.KeyCtrl ? midi_editor::select_mode::add : midi_editor::select_mode::replace);
			...
			if (gesture != gesture_kind::none)
			{
				...
				else if (gesture == gesture_kind::keyboard_pan)
				{
					...
					lane_height = std::clamp(origin.y + size.y - mouse.y, 40.f, size.y * .55f);
				else if (gesture == gesture_kind::draw)
		...
		auto position = static_cast<double>(start);
		...
		if (begin_folded_window("MIDI editor", open, ImGuiWindowFlags_NoScrollWithMouse))
		{
			...
			else
			{
				...
				if (!busy && document->is_file_loaded())
					...
					ImGui::TextWrapped("Open a MIDI file to begin. Unsaved edits can play and render through the shared player.");
			}
			if (ask_discard)
			{
