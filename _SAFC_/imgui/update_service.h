...
using transport =
	std::function<std::vector<std::byte>(std::wstring_view, std::size_t, std::stop_token, const progress_callback&)>;
using installer = std::function<bool(
	const std::filesystem::path&, const std::filesystem::path&, const std::filesystem::path&, std::string&)>;
struct source
{
...
bool extract_executable(
	std::span<const std::byte> archive, const std::filesystem::path& output, std::stop_token stop, std::string& error);
