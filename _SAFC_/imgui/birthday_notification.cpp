...
birthday_notification::birthday_notification(int year, int month, int day)
    : years_old_(month == 8 && day == 31 && year >= 2018 ? year - 2018 : -1),
      open_(active())
{
}
...
void birthday_notification::draw()
{
    ...
    if (begin_folded_window("SAFC birthday", &open_, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoResize))
    {
        ...
        const float rotation = static_cast<float>(std::fmod(ImGui::GetTime(), 2.25)) * turn / 2.25f;
        for (int i = 0; i < 50; ++i)
        {
            const float a = rotation + turn * i / 50.f;
            const float b = rotation + turn * (i + .8f) / 50.f;
            ...
            draw->AddLine(from, to, IM_COL32(255, 127 + 128 * i / 50, 63 + 192 * i / 50, 255), 3.f * scale);
        }
        ImGui::Dummy({40.f * scale, 40.f * scale});
        ImGui::SameLine();
        ImGui::BeginGroup();
        ImGui::TextWrapped("Interesting fact: today is exactly %d years since first SAFC release.\n(o w o  )", years_old_);
        ImGui::Spacing();
        if (ImGui::Button("OK", {80.f * scale, 0})) open_ = false;
        ImGui::EndGroup();
    }
    end_folded_window();
}
