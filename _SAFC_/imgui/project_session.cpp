...
{
	if (loading() || merging() || data.files.empty() || data.save_path.empty())
		return false;
	...
	{
		if (file.key_map)
			file.key_map = std::make_shared<cut_and_transpose>(*file.key_map);

		if (file.volume_map)
			file.volume_map = std::make_shared<decltype(file.volume_map)::element_type>(*file.volume_map);

		if (file.pitch_bend_map)
			file.pitch_bend_map = std::make_shared<decltype(file.pitch_bend_map)::element_type>(*file.pitch_bend_map);
	}

	{
		std::lock_guard lock(state_->mutex);
		state_->progress = {};
		state_->progress.busy = true;
	}
	...
	try
	{
		state_->merger = std::jthread([s = state_.get(), snapshot = std::move(snapshot)](std::stop_token stop) mutable
		{
			...
			try
			{
				...
				s->status("Processing MIDI files");
				...
				if (retain)
				{
					...
					{
						owns_work = false;
						s->status("Merge complete. Intermediate files: " + utf8_path(retained));
					}
					else
					{
						owns_work = false;
						s->status("Merge complete. Intermediate files: " + utf8_path(work));
					}
				}
				else
					s->status("Merge complete");
			}
			...
			{
				std::lock_guard lock(s->mutex);
				s->progress.busy = false;
				s->progress.seconds = std::chrono::duration<double>(std::chrono::steady_clock::now() - began).count();
			}
	...
	{
		state_->merging.store(false, std::memory_order_release);
		std::lock_guard lock(state_->mutex);
		state_->progress.busy = false;
		throw;
	}
...
bool project_session::run_smoke(const std::wstring& directory, std::string& report)
{
	try
	{
...

		{
			...
			out.write(reinterpret_cast<const char*>(midi), sizeof(midi));
		}
		...
		poll();
		if (data.files.size() != 2 || id_at(0) == id_at(1))
			throw std::runtime_error("Project did not assign stable unique file identities");

		const auto survivor = id_at(1);
		remove({id_at(0)});

		if (!find(survivor) || data.files.size() != 1)
			throw std::runtime_error("Project selection changed identity after removal");
		...
		if (!start_merge())
			throw std::runtime_error("Merge did not start");
		...
		if (!check.is_midi)
			throw std::runtime_error("Merge result is not a MIDI");
		...
		if (original_input != std::string(reinterpret_cast<const char*>(midi), sizeof(midi)))
			throw std::runtime_error("Merge modified its source input");

		const auto saved_bytes = bytes;
		if (!start_merge())
			throw std::runtime_error("Cancellation merge did not start");
		...
		if (progress().stage != "Merge cancelled" || read_output() != saved_bytes)
			throw std::runtime_error("Cancelled merge changed the destination");
		...
		if (!start_merge())
			throw std::runtime_error("Failure test did not start");

		while (merging())
			std::this_thread::sleep_for(std::chrono::milliseconds(5));

		data.files[0].filename = old_filename;
		if (progress().error.empty() || read_output() != saved_bytes)
			throw std::runtime_error("Failed merge changed the destination");
		...
		if (!start_merge())
			throw std::runtime_error("Retained-parts merge did not start");
		...
		for (const auto& entry : std::filesystem::directory_iterator(directory))
		{
			...
			if (entry.path().filename().wstring().starts_with(L"project-merged.mid.parts-") &&
				std::filesystem::exists(entry.path() / "0-project-input.mid_.mid"))
				retained = true;
		}
