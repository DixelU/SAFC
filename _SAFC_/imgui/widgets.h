...
{
// Keep enum state in the model; ImGui's Combo API edits an integer index.
template<class Enum>
	requires std::is_enum_v<Enum>
bool enum_combo(const char* label, Enum& value, const char* items)
{
...
inline bool literal_selectable(
	const char* id, std::string_view text, bool selected = false, ImGuiSelectableFlags flags = 0, ImVec2 size = {})
{
	...
	if (size.y == 0)
		size.y = (std::max)(text_size.y, ImGui::GetTextLineHeight());
	const bool pressed = ImGui::Selectable(id, selected, flags, size);
	if (!text.empty() && ImGui::IsItemVisible())
		ImGui::GetWindowDrawList()->AddText(
			position, ImGui::GetColorU32(ImGuiCol_Text), text.data(), text.data() + text.size());
	return pressed;
}
...
inline bool begin_literal_combo(const char* id, std::string_view preview)
{
	...
	const ImVec4 clip{
		position.x + padding.x, position.y + padding.y, position.x + width - height, position.y + height - padding.y};
	if (!preview.empty() && clip.z > clip.x && clip.w > clip.y)
		parent_draw->AddText(
			nullptr, 0.f, {clip.x, clip.y}, color, preview.data(), preview.data() + preview.size(), 0.f, &clip);
	return opened;
}
}
