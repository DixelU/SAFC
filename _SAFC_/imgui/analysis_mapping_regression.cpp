...
void wait_for(Predicate&& predicate)
{
	...
	while (!predicate())
	{
		if (std::chrono::steady_clock::now() > end)
			throw std::runtime_error("Timed out waiting for owned worker.");
...
void write_cancellation_fixture(const std::filesystem::path& path)
{
	...
	const unsigned char header[] = {'M', 'T', 'h', 'd', 0, 0, 0, 6, 0, 0, 0, 1, 1, 0xE0, 
		'M', 'T', 'r', 'k', 
		static_cast<unsigned char>(size >> 24), static_cast<unsigned char>(size >> 16), static_cast<unsigned char>(size >> 8), static_cast<unsigned char>(size)}; 
	...
	for (unsigned i = 0; i < count; ++i)
		output.write(reinterpret_cast<const char*>(notes), sizeof(notes));
...
void check_processing_map_snapshots(const std::filesystem::path& path)
{
	...
	file.volume_map->insert(64, 32);
	file.volume_map->insert(128, 96);
	file.pitch_bend_map = std::make_shared<panel::pitch_curve>();
	file.pitch_bend_map->insert(4096, 0);
	file.pitch_bend_map->insert(8192, 8192);
	...
	*file.key_map = cut_and_transpose(0, 255, 12);
	...
	file.volume_map->insert(0, 0);
	file.volume_map->insert(255, 0);
	file.pitch_bend_map->clear();
	file.pitch_bend_map->insert(0, 0);
	file.pitch_bend_map->insert(16383, 0);
...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		for (std::int64_t tick = 0; tick <= 1440; ++tick)
			require(result->seconds_to_ticks(result->ticks_to_seconds(tick)) == tick,
			    "Tick/time roundtrip lost a tick to floating-point rounding.");
		require(!result->source->internal_time_map.empty(), "Time map missing.");

		const auto csv = directory / "combined.csv";
		require(panel.export_to(csv.wstring(), analysis_panel::export_kind::combined_csv, ","),
		    "Could not start CSV export.");
		...
		require(panel.export_to(csv.wstring(), analysis_panel::export_kind::tempo_csv),
		    "Could not start overwrite export.");
		wait_for([&] { return !panel.export_busy(); });
		require(panel.export_status().starts_with("Export saved:"), panel.export_status().c_str());

		const auto binary = directory / "combined.atraw";
		require(panel.export_to(binary.wstring(), analysis_panel::export_kind::combined_binary),
		    "Could not start ATRAW export.");
		wait_for([&] { return !panel.export_busy(); });
		require(panel.export_status().starts_with("Export saved:"), panel.export_status().c_str());
		require(std::filesystem::file_size(binary) % 32 == 0 && std::filesystem::file_size(binary) >= 160,
		    "ATRAW row size changed.");
		...
		while (raw)
		{
			...
			raw.read(reinterpret_cast<char*>(&tick), 8);
			raw.read(reinterpret_cast<char*>(&poly), 8);
			raw.read(reinterpret_cast<char*>(&seconds), 8);
			raw.read(reinterpret_cast<char*>(&tempo), 8);
			if (raw && tick == 720)
			{
				found = poly == 1 && seconds == 1. && tempo == 60.;
			}
		}
		require(found, "ATRAW values differ from CSV.");

		const auto midi_size = std::filesystem::file_size(path);
		require(panel.export_to(path.wstring(), analysis_panel::export_kind::combined_csv),
		    "Could not start input-collision validation.");
		wait_for([&] { return !panel.export_busy(); });
		...
		volume->insert(0, 0);
		volume->insert(127, 127);
		auto pitch = std::make_shared<mapping_panel::pitch_curve>();
		pitch->insert(0, 0);
		pitch->insert(8192, 8192);
		pitch->insert(16383, 16383);
		bool opened = true;
			...
			for (const auto* list : ImGui::GetDrawData()->CmdLists)
				for (const auto& vertex : list->VtxBuffer)
					require(std::isfinite(vertex.pos.x) && std::isfinite(vertex.pos.y),
					    "A graph produced non-finite geometry.");
		}
		...
		for (auto* window : ImGui::GetCurrentContext()->Windows)
			if (window->ParentWindow && std::string_view(window->ParentWindow->Name) == "MIDI analysis" &&
			    std::string_view(window->Name).find("##folded-content") != std::string_view::npos)
				content = window;
		...
		constexpr const char* controls[] = {"Tempo", "Polyphony", "Notes/sec", "Seconds on horizontal axis",
		    "Auto vertical scale", "From", "To", "Fit all", "Ticks", "Seconds", "Delimiter", "Format", "Ticks -> time",
		    "Time -> ticks", "Export tempo CSV", "Export combined data", "Export notes/sec CSV", "##tempo-plot",
		    "##polyphony-plot", "##nps-plot"};
		...
		ImGui::ActivateItemByID(content->GetID("Auto vertical scale"));
		frame_analysis();
		...
		require(
		    volume->evaluate_as<std::uint8_t>(127).value_or(0) == 127, "Rendering unexpectedly changed volume map.");
		require(pitch->evaluate_as<std::uint16_t>(8192).value_or(0) == 8192,
		    "Pitch center 8192 became an invalid sentinel.");

		const auto cancellation_path = directory / "cancel-analysis.mid";
		write_cancellation_fixture(cancellation_path);
		panel.open_file(cancellation_path.wstring());
		panel.cancel();
		wait_for([&] { return !panel.busy(); });
		...
		wait_for([&] { return !panel.busy(); });
		require(!panel.result() && panel.status().find("nonzero") != std::string::npos,
		    "Invalid MIDI time division was accepted.");
		panel.open_file((directory / "missing.mid").wstring());
		wait_for([&] { return !panel.busy(); });
		require(!panel.result(), "Missing MIDI was accepted.");
		panel.shutdown();
		std::cout << "Analysis/mapping regression passed: project map snapshots, tempo integration, exact graph peaks, "
		             "CSV/ATRAW, overwrite, native draw, cancel/reopen and invalid input.\n";
		return 0;
	}
