...
namespace
{
// Standard MIDI keys, velocities and controllers use seven-bit values.
constexpr int standard_value_count = 128;
constexpr int standard_value_max = standard_value_count - 1;
// SAFC also preserves the legacy extended byte-wide key and velocity maps.
constexpr int extended_value_count = 256;
constexpr int extended_value_max = extended_value_count - 1;
constexpr int pitch_bend_max = 16383;
constexpr int pitch_bend_center = 8192;
constexpr float point_pick_radius = 10.f;
constexpr std::size_t point_marker_limit = 1500;
...
void draw_key_strip(ImDrawList& draw, ImVec2 origin, ImVec2 size, const ::cut_and_transpose& map, int bank, bool output)
{
	...
	for (int key = 0; key < extended_value_count; ++key)
	{
		...
		const float x = origin.x + size.x * (source - bank * standard_value_count) / float(standard_value_count);
		const float next = x + size.x / float(standard_value_count);
		if (next <= origin.x || x >= far.x)
			continue;
		const bool included = source >= 0 && source <= extended_value_max &&
...
void identity(Curve& curve, int maximum)
{
	curve.clear();
	curve.insert(0, 0);
	curve.insert(static_cast<typename Curve::key_type>(maximum), static_cast<typename Curve::value_type>(maximum));
}
...
void mapping_panel::draw_key_map(const char* label, std::shared_ptr<::cut_and_transpose>& map, bool* open)
{
	...
	{
		if (enabled)
			map = std::make_shared<::cut_and_transpose>(0, standard_value_max, 0);
		else
			map.reset();
	}
	if (!map)
	{
		ImGui::TextDisabled("Notes keep their original keys.");
		end_folded_window();
		return;
	}

	if (ImGui::Button("Reset (0..255)"))
		*map = ::cut_and_transpose(0, extended_value_max, 0);
	ImGui::SameLine();
	if (ImGui::Button("Keep 0..127"))
		*map = ::cut_and_transpose(0, standard_value_max, 0);
	ImGui::SameLine();
	if (ImGui::Button("0..127 -> 128..255"))
		*map = ::cut_and_transpose(0, standard_value_max, standard_value_count);
	ImGui::SameLine();
	...
	ImGui::TextDisabled(
		"Drag across an input keyboard to set the cut range. A/D: first key, Q/E: last key, W/S: transpose.");
	...
	for (int bank = 0; bank < banks; ++bank)
	{
		...
		ImGui::Text("Input %d..%d", bank * standard_value_count, bank * standard_value_count + standard_value_max);
		...
		{
			const int key = bank * standard_value_count +
				std::clamp(
					int((ImGui::GetIO().MousePos.x - origin.x) / size.x * standard_value_count), 0, standard_value_max);
			...
			if (result != ::cut_and_transpose::rejected)
				ImGui::SetTooltip(
					"%s (%d) -> %s (%u)", key_name(key).c_str(), key, key_name(result).c_str(), unsigned(result));
			else
				ImGui::SetTooltip("%s (%d) is removed", key_name(key).c_str(), key);
		}
		ImGui::PopID();
	}
	...
	int kept = 0;
	for (int key = 0; key <= standard_value_max; ++key)
		kept += map->process(static_cast<std::uint8_t>(key)) != ::cut_and_transpose::rejected;
	ImGui::Text("%d of 128 standard keys retained; output range %d..%d", kept,
		std::max(0, int(map->min_val) + map->transpose_val),
		std::min(extended_value_max, int(map->max_val) + map->transpose_val));
	end_folded_window();
}

template<class Curve>
void mapping_panel::draw_curve_tools(Curve& map, curve_state& state, std::optional<Curve>& clipboard, int maximum)
{
	using key_type = typename Curve::key_type;
	using value_type = typename Curve::value_type;
	if (ImGui::Button("Identity"))
		identity(map, maximum);
	ImGui::SameLine();
	...
	if (ImGui::Button("Trace"))
	{
		...
		for (int i = 0; i <= maximum; ++i)
			map.insert(static_cast<key_type>(i),
				original
	...
	{
		ImGui::Checkbox("0..255", &state.extended);
		ImGui::SameLine();
	}
	else
	{
		ImGui::Checkbox("Keep center", &state.symmetric);
		ImGui::SameLine();
	}
	if (ImGui::Button("Generate power curve"))
	{
		const int limit = (maximum == extended_value_max && !state.extended) ? standard_value_max : maximum;
		...
		for (int i = 0; i <= limit; ++i)
		{
			...
			if (maximum > extended_value_max && state.symmetric)
			{
				const double center = pitch_bend_center;
				const double range = i < center ? center : maximum - center;
				value =
					center + std::copysign(std::pow(std::abs(i - center) / range, state.degree) * range, i - center);
			}
			map.insert(
				static_cast<key_type>(i), static_cast<value_type>(std::clamp(std::ceil(value), 0., double(limit))));
		}
		simplify(map);
	}
	if (ImGui::RadioButton("Single point", state.mode == curve_mode::point))
		state.mode = curve_mode::point;
	ImGui::SameLine();
	if (ImGui::RadioButton("Two-point segment", state.mode == curve_mode::segment))
		state.mode = curve_mode::segment;
	ImGui::SameLine();
...
void mapping_panel::edit_curve_point(Curve& map, curve_state& state, int x, int y)
{
	...
	if (state.mode == curve_mode::segment)
	{
		const int first = std::min(x, state.first_x);
		const int last = std::max(x, state.first_x);
		auto point = map.points().lower_bound(static_cast<key_type>(first));
		while (point != map.points().end() && point->first <= last)
		{
			// The converter owns a std::map: only the erased iterator is invalidated.
...
template<class Curve>
void mapping_panel::draw_curve_canvas(Curve& map, curve_state& state, int maximum)
{
	using key_type = typename Curve::key_type;
	...
	{
		return ImVec2(origin.x + float(x / maximum) * size.x, far.y - float(y / maximum) * size.y);
	};
	draw->AddRectFilled(origin, far, IM_COL32(6, 19, 30, 255));
	if (maximum == extended_value_max)
		draw->AddRectFilled(point(0, standard_value_max), point(standard_value_max, 0), IM_COL32(22, 53, 46, 180));
	for (int i = 1; i < 8; ++i)
	{
		draw->AddLine(ImVec2(origin.x + size.x * i / 8, origin.y), ImVec2(origin.x + size.x * i / 8, far.y),
			IM_COL32(33, 54, 73, 150));
		draw->AddLine(ImVec2(origin.x, origin.y + size.y * i / 8), ImVec2(far.x, origin.y + size.y * i / 8),
			IM_COL32(33, 54, 73, 150));
	}
	...
	for (int pixel = 0, pixels = std::max(2, int(size.x)); pixel <= pixels; ++pixel)
	{
		...
		if (have_previous)
			draw->AddLine(previous, current, curve_color, 1.7f);
		previous = current;
		have_previous = true;
	}
	const auto mouse = ImGui::GetIO().MousePos;
	int nearest = -1;
	float nearest_distance = point_pick_radius * point_pick_radius;
	for (const auto& [x, y] : map.points())
	{
		...
		if (map.size() < point_marker_limit)
			draw->AddCircleFilled(p, x == state.x ? 4.f : 2.5f, IM_COL32(220, 231, 240, 255));
	}
	if (ImGui::IsItemHovered() || ImGui::IsItemActive())
	{
		...
		if (state.first_x >= 0 && state.mode == curve_mode::segment)
			draw->AddLine(point(state.first_x, state.first_y), point(x, y), accent, 2.f);
...
template<class Curve>
void mapping_panel::draw_curve_points(Curve& map, curve_state& state, int maximum)
{
	using key_type = typename Curve::key_type;
	using value_type = typename Curve::value_type;
	...
	state.y = std::clamp(state.y, 0, maximum);
	if (ImGui::Button("Set point"))
		map.insert(static_cast<key_type>(state.x), static_cast<value_type>(state.y));
	ImGui::SameLine();
	if (ImGui::Button("Delete point"))
		(void)map.erase(static_cast<key_type>(state.x));
	if (ImGui::CollapsingHeader("Control points"))
	{
		...
		if (ImGui::BeginChild("point_list", ImVec2(0, 140), ImGuiChildFlags_Borders))
		{
			...
			while (clipper.Step())
			{
				std::advance(point, clipper.DisplayStart - row);
				for (row = clipper.DisplayStart; row < clipper.DisplayEnd; ++row, ++point)
				{
					const auto [x, y] = *point;
					const auto text = std::to_string(x) + " -> " + std::to_string(y);
					if (ImGui::Selectable(text.c_str(), state.x == x))
					{
						state.x = x;
						state.y = y;
					}
	...
	curve_state& state, std::optional<Curve>& clipboard, int maximum)
{
	...
	if (!map)
	{
		ImGui::TextDisabled("Values pass through unchanged.");
		end_folded_window();
		return;
	}
	if (state.owner != map.get())
	{
		state.owner = map.get();
		state.first_x = -1;
	}

	draw_curve_tools(*map, state, clipboard, maximum);
	draw_curve_canvas(*map, state, maximum);
	draw_curve_points(*map, state, maximum);
	end_folded_window();
}

void mapping_panel::draw_volume_map(const char* label, std::shared_ptr<volume_curve>& map, bool* open)
{
	draw_curve("Volume mapping", label, map, open, volume_, volume_clipboard_, extended_value_max);
}

void mapping_panel::draw_pitch_map(const char* label, std::shared_ptr<pitch_curve>& map, bool* open)
{
	draw_curve("Pitch bend mapping", label, map, open, pitch_, pitch_clipboard_, pitch_bend_max);
}
}
