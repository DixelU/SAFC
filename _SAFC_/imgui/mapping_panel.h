...
private:
	...
	void draw_curve(const char* title, const char* label, std::shared_ptr<Curve>& map, bool* open, curve_state& state,
		std::optional<Curve>& clipboard, int maximum);

	template<class Curve>
	void draw_curve_tools(Curve& map, curve_state& state, std::optional<Curve>& clipboard, int maximum);
	template<class Curve>
	void draw_curve_canvas(Curve& map, curve_state& state, int maximum);
	template<class Curve>
