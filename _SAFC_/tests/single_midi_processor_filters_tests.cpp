...
void test_flattening_large_spans()
{
    auto settings = make_settings();
    settings.flatten = true;
    ...
    for (const processor::tick_type begin : {processor::tick_type{0}, span})
    {
        ...
        settings.original_time_map[begin] = {uint128_t(begin) * tempo, 480000000};
        settings.original_time_map[begin + span] = {uint128_t(begin + span) * tempo, 480000000};
        auto bundle = processor::filters_constructor(settings);
        for (const auto delta : {span / 4, span / 2, span - 1})
        {
