...
struct cut_and_transpose
{
	...
	cut_and_transpose(std::uint8_t min, std::uint8_t max_val, std::int16_t transpose_val) noexcept :
		min_val(min), max_val(max_val), transpose_val(transpose_val)
	{}

	inline std::optional<std::uint8_t> process(std::uint8_t value) const noexcept
	{
