...
using EventTransformingFilter = std::function<bool(DataIterator cur, DataIterator data_end)>;
...
#define PCLOG true
	...
	template<typename T>
	static std::uint64_t& get_value(std::vector<std::uint8_t>& vec, size_t index)
	{
		return *(T*)&vec[index];
	}
	...
	static bool put_data_in_buffer(
		bbb_ffr& file_input,
	...
	)
	{
		...
		while (file_input.good() && is_good)
		{
			...
			{
				is_good = false;
				messages.ErrorLine = "Unexpected 0 RSB";
				break;
			}
			...
			switch (command)
			{
				MH_CASE(0x80) : MH_CASE(0x90) :
				{
					...
					else if(current_polyphony_object.size())
					{
						...
						else
						{
							...
							messages.WarningLine = "Incorrect index of note reference " + std::to_string(other_note_reference);
						}
					}
					else
					{
						polyphony_error = true;
						++noteoff_misses;
						messages.WarningLine = "OFF of nonON Note: " + std::to_string(noteoff_misses);
					}
				...
				case 0xFF:
				{
					...
					push_back<size_t>(data_buffer, type);
				...
				default:
				{
					messages.ErrorLine = "Unknown event type " + std::to_string(command);
					break;
				}
			}
