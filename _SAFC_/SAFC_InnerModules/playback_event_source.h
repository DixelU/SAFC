...
struct playback_event_source
{
	...
	// Fast-forward to target_us. The default rewinds and lets simple_player walk
	// the prefix, reconstruct channel state, and re-strike notes held at the
	// target. An indexed override may jump over the prefix, but must then yield
	// reconstructed controller state and held note-ons at target_us before later
	// events. Implies rewind().
