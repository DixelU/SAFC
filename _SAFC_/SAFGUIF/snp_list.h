...
struct selectable_properted_list : handleable_ui_part
{
	...
	selectable_properted_list(button_settings* butt_settings,
		...
		float width,
		float space_between,
		std::uint32_t max_chars_in_line = 0,
		std::uint32_t max_visible_lines = 0,
	...
	void safe_push_back_new_string(std::string button_text)
	{
		...
		if (max_chars_in_line)
			button_text = button_text.substr(0, max_chars_in_line);
