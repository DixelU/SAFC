...
namespace
{
	...
	void center_checkbox_tip(checkbox* control)
	{
		if (control && control->tip)
			control->tip->safe_change_position_argumented(
				_Align::center, 0, control->y_pos - control->side_size);
	}
}
	...
	unsigned background_color, unsigned header_color, unsigned border_color)
{
	...
	(*window)["CENTER"] = new wheel_variable_changer([](double) { preview_editor_lfo(); }, -70, 65, 64, 1,
		system_white, "Value", "Step", wheel_variable_changer::Type::addition);
	(*window)["RANGE"] = new wheel_variable_changer([](double) { preview_editor_lfo(); }, 70, 65, 63, 1,
		system_white, "Range", "Step", wheel_variable_changer::Type::addition);
	(*window)["CYCLES"] = new wheel_variable_changer([](double) { preview_editor_lfo(); }, -70, 0, 1, 2,
